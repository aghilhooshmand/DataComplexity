...
_N3_FORK_DM = None
_N3_FORK_Y = None
_N3_FORK_CLASSES = None
_N3_FORK_K = 1


def _n3_chunk_ranges(n, n_jobs):
    if n_jobs <= 1 or n <= 1:
        return [(0, n)]
    n_jobs = min(int(n_jobs), n)
    base, extra = divmod(n, n_jobs)
    ranges = []
    start = 0
...
def _pycol_fork_knn_row(inx, line, k, y, classes, clear_diag=True):
    line = np.asarray(line, dtype=float).copy()
    count = np.zeros(len(classes))
    if clear_diag:
        line[inx] = math.inf
    for _ in range(int(k)):
        index = int(np.argmin(line))
        line[index] = math.inf
        cls_inx = int(np.where(classes == y[index])[0][0])
        count[cls_inx] += 1
    return count


def _pycol_fork_n3_chunk(args):
    start, end = args
    global _N3_FORK_DM, _N3_FORK_Y, _N3_FORK_CLASSES, _N3_FORK_K
    dm = _N3_FORK_DM
    y = _N3_FORK_Y
    classes = _N3_FORK_CLASSES
    k = int(_N3_FORK_K)
    n3_counts = np.zeros(len(classes), dtype=float)
    inst_hardness = []
    for sample in range(start, end):
        count = _pycol_fork_knn_row(sample, dm[sample].copy(), k, y, classes, clear_diag=True)
        cls_inx = int(np.where(classes == y[sample])[0][0])
        class_count = count[cls_inx]
        max_count = np.max(count)
        if class_count < max_count:
            n3_counts[cls_inx] += 1
        inst_hardness.append(float(1 - class_count / k))
    return n3_counts, inst_hardness
...
class Complexity:
    ...
    def N3(self, k=1, imb=False, inst_level=False, n_jobs=1):
        '''
        Calculate the N3 value complexity measure in [1].
        n_jobs > 1: parallel row chunks via fork (Linux; not with inst_level=True).
        '''
        nj = int(n_jobs) if n_jobs is not None else 1
        use_parallel = nj > 1 and not inst_level and sys.platform != "win32"
        if use_parallel:
            try:
        ...
        if use_parallel:
            global _N3_FORK_DM, _N3_FORK_Y, _N3_FORK_CLASSES, _N3_FORK_K
            _N3_FORK_DM = self.dist_matrix
            _N3_FORK_Y = self.y
            _N3_FORK_CLASSES = self.classes
            _N3_FORK_K = int(k)
            ranges = _n3_chunk_ranges(len(self.X), nj)
            with ctx.Pool(nj) as pool:
                parts = pool.map(_pycol_fork_n3_chunk, ranges)
            n3_counts = np.zeros(len(self.classes), dtype=float)
            inst_hardness = []
            for part_counts, part_hard in parts:
                n3_counts += part_counts
                inst_hardness.extend(part_hard)
        else:
